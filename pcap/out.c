...
)
{
    ...
    // check for keywork password in payload
    if (payload_length > 0) {
        const u_char *temp_pointer = payload;

        const u_char *str = payload;
        int count1 = payload_length;
...
int main(int argc, char **argv) {
    ...
    handle = pcap_open_live(device, snapshot_length, 0, 500, error_buffer);
